...
- **Otherwise (Cline or any other non-Codex runtime):** Run plan **inline** (do NOT background) so the plan-checker runs. The next phase's discuss does not overlap planning here — correctness over overlap.
...
- **Otherwise (Cline or any other non-Codex runtime):** Run execute **inline** (do NOT background) so worktree isolation and verification run:
...
Display `Phase ${PHASE_NUM} ✅ ${PHASE_NAME} — Verification passed`, run `@D:/projects/vietnga/BoNaoTriThuc/company-wiki/gsd-core/workflows/transition.md`, then Proceed to iterate step.
...
On "All good — continue": set VERIFICATION frontmatter `status: passed`, display `Phase ${PHASE_NUM} ✅ Human validation passed`, run `@D:/projects/vietnga/BoNaoTriThuc/company-wiki/gsd-core/workflows/transition.md`, then iterate.
...
Read and execute: `D:/projects/vietnga/BoNaoTriThuc/company-wiki/gsd-core/references/autonomous-smart-discuss.md`
...
This means the user is always answering discuss questions (lightweight, interactive) while the heavy work (planning, code generation) runs in the background. The main context only accumulates discuss conversations — plan and execute contexts are isolated in their agents. (On Cline and all other non-Codex runtimes, plan and execute run inline, so they run sequentially and their work accumulates in the main context.)
